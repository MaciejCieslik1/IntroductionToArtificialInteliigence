...
class DecisionTreeClassifier(object):
    ...
    def build_tree(self, data: np.ndarray, classes: np.ndarray[int], depth: int = 0) -> Node:
        if depth < self.max_depth:
            ...
            if not best_split or len(best_split["lower_classes"]) == 0 or len(best_split["higher_classes"]) == 0:
                return self.count_most_common_class(classes, depth)
    ...
    def count_most_common_class(self, classes: np.ndarray[int], depth: int) -> int:
...
def test(test_size: float, random_state: int = None):
    iris = load_iris()
    x = iris.data
    y = iris.target
        ...
        for iteration in range(NUMBER_OF_REPEATS):
            ...
            for sample, gt in zip(x_test, y_test):
                ...
                if prediction == gt:
                    correct_counter += 1
                if iteration == 0:
                    print(f"predicted class: {prediction}, actual class: {gt}, iteration {iteration}")
            average_accuracy += correct_counter / len(y_test)
        average_accuracy = average_accuracy / NUMBER_OF_REPEATS
...
def generate_plot(dephts: list[int], accuracies: list[float], test_size: float, random_state: int = None):
