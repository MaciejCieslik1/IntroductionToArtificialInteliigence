...
class ConnectFourState(State):
    ...
    def undo_move(self, move: Move) -> 'ConnectFourState':
        if move:
            for i in range(len(self.fields[move.column]) - 1, -1, -1):
                if self.fields[move.column][i] is not None:
                    self.fields[move.column][i] = None
                    break
        return ConnectFourState(current_player=self._other_player, other_player=self._current_player, fields=self.fields)
    ...
    def get_score(self, player: Player) -> float:
        ...
        score = 0
        for n in N:
            for column_id in range(len(self.fields)):
                for start_row_id in range(len(self.fields[column_id]) - (n - 1)):
                    is_sequence = self._check_n((column_id, start_row_id), (0, 1), player, n)
                    if is_sequence:
                        score += n

            for start_column_id in range(len(self.fields) - (n - 1)):
                for row_id in range(len(self.fields[start_column_id])):
                    is_sequence = self._check_n((start_column_id, row_id), (1, 0), player, n)
                    if is_sequence:
                        score += n

            for start_column_id in range(len(self.fields) - (n - 1)):
                for start_row_id in range(len(self.fields[start_column_id]) - (n - 1)):
                    is_sequence_tuple = [
                        self._check_n((start_column_id, start_row_id), (1, 1), player, n) or
                        self._check_n((start_column_id, start_row_id + (n - 1)), (1, -1), player, n)
                    ]
                    if True in is_sequence_tuple:
                        score += n
        return score
    ...
    def _check_n(self, start_coords: Tuple[int, int], move_coords: Tuple[int, int], player: Player,  n: int) -> bool:
        fields = {
            self.fields[start_coords[0] + move_coords[0] * i][start_coords[1] + move_coords[1] * i] for i in range(n)
        }
        if len(fields) == 1 and next(iter(fields)) == player:
            return True
        return False
